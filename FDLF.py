...
def FDLF(bus_data, line_data, Sbase, Ubase, max_iterations, tolerance, Q_lim, iterate_partial_orEnd, XR_ratio):
    ...
    sump, sumq, flow, S_I_injections, flow_pu= PowerLossAndFlow(line_data, BusList, Sbase, Ubase, XR_ratio)
    df_FDLF, df_FDLF_pu = makeDataFrame(BusList, Sbase, Ubase)
    FDLF = df_FDLF.to_latex()
    print(FDLF)
    print("\n")
    FDLF_pu = df_FDLF_pu.to_latex()
    print(FDLF_pu)
    print("\n")    
    flow2 = flow.to_latex()
    print(flow2)
