...
Line_pq.loc[self.bus_q, self.bus_p] = complex(-y_pq)
            ...
            bus = {
                'Bus ID' : self.bus_id,
                'Type' : 'PV',
                'Known 1' : f'P_{self.bus_id}',
                'Known 1 Value' : self.P,
                'Known 2' : f'|v_{self.bus_id}|',
                'Known 2 Value' : self.voltage_magnitude,
                'Unknown 1' : f'Q_{self.bus_id}',
                'Unknown 1 Value' : self.Q,
                'Unknown 2' : f'delta_{self.bus_id}',
                'Unknown 2 Value' : self.voltage_angle
            }
        elif (self.gen_MW and self.gen_MVAr) == -1:
            # Slack bus: dv, 
            bus = {
                'Bus ID' : self.bus_id,
                'Type' : 'SLACK',
            ...
            bus = {
                ...
                'Known 1' : f'P_{self.bus_id}',
                'Known 1 Value' : self.P,
                'Known 2' : f'Q_{self.bus_id}',
                'Known 2 Value' : self.Q,
                'Unknown 1' : f'|v_{self.bus_id}|',
                'Unknown 1 Value' : self.voltage_magnitude,
                'Unknown 2' : f'delta_{self.bus_id}',
                'Unknown 2 Value' : self.voltage_angle
            }
